...
window.onload = function () {
  ...
  $(".img-container").append(
    '<button style ="background: #9e00c5;" class="btn btn-success btn-add-cart"><span class="glyphicon glyphicon-shopping-cart"></span> to cart</button>'
  );
...
//     $(item).find("p")[1].textContent =
//       "$" +
//       (Math.floor(Math.random() * (199 - 0)) + 0) +
//       "." +
...
function addToCart(name, price) {
  ...
  newDiv.append(
    '<button class="btn btn-danger btn-xs" onclick="deleteItem(this)">X</button>'
  );
  newDiv.append("<hr>");
...
function updatePrice() {
  ...
  let newDiv = $("<div></div>");
  newDiv.append("<strong>Subtotal السعر : LE " + subTotal.toFixed(2) + "<br>");
  newDiv.append("<strong>ضريبة Tax : " + tax * 100 + "%<br>");
  newDiv.append(
    "<strong>Total Price السعر النهائي : LE " +
      (subTotal + subTotal * tax).toFixed(2) +
      "</strong>"
  );

  newDiv.append(
    '<button class="btn btn-info btn-block" onclick="openModal()">Continue</button>'
  );
...
function cartToString() {
  ...
  itemsString += "</p><p>Subtotal: LE" + subTotal.toFixed(2) + "<br>";
  itemsString += "Tax: " + tax * 100 + "%<br>";
  itemsString +=
    "Total price with tax: <mark><strong> LE" +
    (subTotal + subTotal * tax).toFixed(2) +
    "</strong></mark></p></small>";
  return itemsString;
}
...
function formSubmitted() {
  if (!checkEmptyFields()) {
    ...
    sweetAlert(
      "Thanks! ",
      "Thanks for your order! We will contact you for more information",
      "success"
    );
  } else {
    return;
...
function checkEmptyFields() {
  ...
  if (!$.trim($("#formPhone").val()).length) {
    $("#formPhone").animateCss("animated jello");
    somethingEmpty = true;
  }
